...
// A cookie value is whatever the browser sends back, and a stray percent sign is enough to make
// decodeURIComponent throw. On the API routes that landed in the error handler as a 500, and the
// browser holding the bad cookie kept sending it, so the dashboard stayed broken for that browser
// until the jar was cleared by hand. On the serve path it was worse: unlockValid runs outside the
// async error path, so one unauthenticated request to a private or password artifact took the
// whole process down and every artifact on the host with it. A value that does not decode is
// treated as an absent cookie.
...
export async function createAuthStore(storage, { apiKey, baseUrl, oidc = false }) {
  ...
  // `extra` rides in the cookie payload; the company sign-in (oidc.js) marks its sessions
  // { oidc: true } so they are not tied to the local admin record.
  ...
  function sessionPrincipal(req) {
    ...
    // Every cookie this server mints carries a real number here, so anything else is refused.
    // The old read let a payload with no exp, or a null one, skip the lapse check entirely,
    // which is the shape T1.2.12 fixed on verifyCapToken. Nothing reaches it today, since
    // issueSession stamps exp from a constant and forging a payload needs adminSecret. Making
    // the TTL configurable is what would reopen it.
    //
    // Number.isFinite rather than a typeof test, because typeof NaN and typeof Infinity are
    // both "number" and neither compares as lapsed. JSON writes both as null, so no cookie can
    // carry one, and the point is that a later caller reaching this with a live object cannot
    // either. verifyCapToken and unlockValid in server.js read the same way.
    if (!Number.isFinite(payload.exp) || payload.exp <= Date.now()) return null;
    // A company sign-in session is the IdP's word, not the local admin record's: it stays
    // valid only while OIDC is configured, so unsetting OIDC_ISSUER revokes every such session.
  ...
  // Does this request carry a credential this server accepts? No scope check, no lastUsedAt
  // stamp, no answer written: it only says whether somebody is behind the request. The
  // body-size gate in server.js asks before any body is read, because an anonymous caller must
  // not be able to make the server buffer a publish-sized body, and must not be able to spend
  // the operator's publish budget. Both reads are in memory, so this costs a hash.
  ...
  function verifyCapToken(token, slug, epoch) {
    ...
    // Tokens minted while CAP_TOKEN_TTL_DAYS held junk carry null
    ...
    // Number.isFinite for the reason spelled out on sessionPrincipal: Infinity is a number
    // and is never in the past.
